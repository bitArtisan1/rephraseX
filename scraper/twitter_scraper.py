...
class Twitter_Scraper:
    def __init__(
        ...
        self.headless = True  # Track if we're in headless mode
    ...
    def _get_driver(
        ...
        # For Hiding Browser, enable headless mode if self.headless is True
        if self.headless:
    ...
    def restart_driver_in_headful_mode(self):
        print("Restarting WebDriver in headful mode...")
        self.driver.quit()
        self.headless = False  # Disable headless mode
        self.driver = self._get_driver()  # Reinitialize the driver in headful mode
    ...
    def login(self):
        ...
        try:
            ...
            # Detect and handle unusual activity or verification challenge
            if self._detect_unusual_activity():
                print("Unusual activity detected. Switching to manual verification mode.")
                self.restart_driver_in_headful_mode()  # Switch to headful mode
                self.driver.get(TWITTER_LOGIN_URL)
                sleep(3)
                self._input_username()  # Re-enter username after restarting the driver

                input("Please complete the verification in the browser. Press Enter to continue...")
...
"""
    ...
    def _detect_unusual_activity(self):
        try:
            # Check for elements indicating unusual activity or a challenge
